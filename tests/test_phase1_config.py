...
from engine.config import get_config, upgrade_workbook_config
import json


def _make_upgraded_workbook():
    temp_dir = tempfile.TemporaryDirectory()
    workbook = Path(temp_dir.name) / "APS_BF_SMS_RM.test.xlsx"
    shutil.copy2(ROOT / "APS_BF_SMS_RM.xlsx", workbook)
    upgrade_workbook_config(workbook)
    return temp_dir, workbook
...
def test_algorithm_config_sheet():
    ...
    import openpyxl
    temp_dir, workbook = _make_upgraded_workbook()
    ...
    try:
        ...
        assert data_rows >= 56, f"Expected at least 56 parameters after upgrade, got {data_rows}"
...
def test_category_filtering():
    ...
    try:
        ...
        for category, expected_min_count in categories.items():
            params = config.params_by_category(category)
            actual_count = len(params)
            assert actual_count >= expected_min_count, f"Category {category}: expected at least {expected_min_count}, got {actual_count}"
...
def test_all_params_dict():
    ...
    try:
        ...
        assert len(all_params) >= 56, f"Expected at least 56 params after upgrade, got {len(all_params)}"
        ...
        assert "PRIORITY_SEQUENCE" in all_params, "PRIORITY_SEQUENCE not in dict"
        assert "CTP_DECISION_PRECEDENCE_SEQUENCE" in all_params, "CTP_DECISION_PRECEDENCE_SEQUENCE not in dict"
