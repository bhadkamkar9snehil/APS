...
.DESCRIPTION
    This is the project's manual release pipeline. It does, in order:

      1. dotnet test     — runs every test project registered in APS.slnx and stops on failure.
      2. dotnet publish  — publishes APS.DesktopHost using the runtime/self-contained/ReadyToRun
                           settings owned by APS.DesktopHost.csproj.
      3. vpk pack        — wraps that publish output into a Velopack installer/update release.
