...
@app.route('/api/aps/planning/recommend-campaign', methods=['GET'])
def aps_planning_recommend_campaign():
    """Recommend next rolling campaign based on open SOs."""
    try:
        from engine.rolling_campaign import CampaignSelector

        # Get sales orders using the correct API
        result = store.list_rows('sales-orders', filters=[], limit=1000)
        sales_orders = result.get('items', [])

        selector = CampaignSelector(get_config().all_params())
        campaign = selector.recommend_next_campaign(
            sales_orders,
            strategy='URGENT_FIRST'
        )

        if not campaign:
            return _jsonify({"recommended_sos": [], "message": "No open SOs available"})

        return _jsonify({
            "campaign_id": campaign.id,
            "recommended_sos": campaign.sales_orders,
            "grade": campaign.grade,
            "total_mt": campaign.total_mt,
            "estimated_heats": campaign.estimated_heats,
            "estimated_duration_hours": campaign.estimated_total_duration
        })
    except Exception as e:
        return _jsonify({"error": str(e), "traceback": traceback.format_exc()}, 500)


@app.route('/api/aps/planning/simulate-campaign', methods=['POST'])
def aps_planning_simulate_campaign():
    """Simulate a campaign and generate production orders."""
    try:
        from engine.rolling_campaign import CampaignSelector, POGenerator, RollingCampaign

        payload = request.get_json(silent=True) or {}
        sales_orders = payload.get('sales_orders', [])
        po_strategy = payload.get('po_strategy', '1to1')

        if not sales_orders:
            return _jsonify({"error": "No sales orders provided"}, 400)

        # Create campaign from selected SOs
        selector = CampaignSelector(get_config().all_params())
        total_mt = sum(so.get("Order_Qty_MT", 0) for so in sales_orders)
        grade = sales_orders[0].get('Grade') if sales_orders else 'Unknown'
        heats = selector._estimate_heats(sales_orders)
        sms_dur = selector._estimate_sms_duration(sales_orders)
        rm_dur = selector._estimate_rm_duration(sales_orders)
        ...
        campaign = RollingCampaign(
            ...
            estimated_sms_duration=sms_dur,
            estimated_rm_duration=rm_dur,
            estimated_total_duration=sms_dur + rm_dur
        )

        # Generate POs
        po_gen = POGenerator(get_config().all_params())
        pos = po_gen.generate_pos(campaign, strategy=po_strategy)
        campaign.production_orders = pos

        # Simple feasibility check
        all_on_time = all(
            selector._parse_date(so.get('Delivery_Date')) > datetime.now() + timedelta(hours=campaign.estimated_total_duration)
            for so in sales_orders
        )

        on_time_count = sum(
            1 for so in sales_orders
            if selector._parse_date(so.get('Delivery_Date')) > datetime.now() + timedelta(hours=campaign.estimated_total_duration)
        )

        return _jsonify({
            "campaign": campaign.to_dict(),
            "production_orders": [po.to_dict() for po in pos],
            "feasible": all_on_time,
            "on_time_count": on_time_count,
            "late_count": len(sales_orders) - on_time_count,
            "total_count": len(sales_orders),
            "gantt_data": [
                f"SMS: {campaign.estimated_sms_duration:.1f}h",
                f"RM: {campaign.estimated_rm_duration:.1f}h",
                f"Total: {campaign.estimated_total_duration:.1f}h"
            ]
        })
    except Exception as e:
        return _jsonify({"error": str(e), "traceback": traceback.format_exc()}, 500)
...
@app.route('/api/aps/planning/release-campaign', methods=['POST'])
def aps_planning_release_campaign():
    """Release a campaign to operations."""
    try:
        payload = request.get_json(silent=True) or {}
        sales_orders = payload.get('sales_orders', [])
        po_strategy = payload.get('po_strategy', '1to1')
        ...
        # Create campaign and generate POs
        from engine.rolling_campaign import CampaignSelector, POGenerator, RollingCampaign
        ...
        # Update SO statuses in workbook (mark as assigned to this campaign)
        so_ids_set = set(so.get('SO_ID') for so in sales_orders)
        for so_id in so_ids_set:
            try:
                store.update_row('sales-orders', so_id, {'Campaign_ID': campaign_id}, partial=True)
            except Exception:
                pass  # Continue if single update fails

        return _jsonify({
            "campaign_id": campaign.id,
            "status": "RELEASED",
            "production_orders": len(pos),
            "campaign": campaign.to_dict()
        })
    except Exception as e:
        return _jsonify({"error": str(e), "traceback": traceback.format_exc()}, 500)
