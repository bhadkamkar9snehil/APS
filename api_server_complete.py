"""
APS Flask API Server - Complete Backwards-Compatible Edition
------------------------------------------------------------
Reads APS_BF_SMS_RM.xlsx directly (no xlwings/Excel COM needed),
serves the existing APS endpoints, and also exposes generic workbook CRUD
routes for all structured sheets.

Start:  python api_server_complete.py
...
# ── Existing/backwards-compatible routes ─────────────────────────────────────
...
def run_ctp_api():
    try:
        ...
        campaigns = _state["campaigns"] or build_campaigns(
            d["sales_orders"], min_cmt, max_cmt, inventory=d["inventory"], bom=d["bom"], config=config, skus=d["skus"]
        )
...
def orders_collection():
    ...
    so_id = str(body.get("SO_ID") or "").strip()
    if not so_id:
...
# ── New generic workbook CRUD layer ──────────────────────────────────────────
...
def sheet_collection(sheet_name: str):
    ...
    if request.method == "GET":
        ...
        return _jsonify(store.list_rows(sheet_name, search=search, sort_by=sort_by, sort_dir=sort_dir, limit=limit, offset=offset, filters=filters or None))
...
if __name__ == "__main__":
    print(f"\n  APS Complete API -> http://localhost:{PORT}")
